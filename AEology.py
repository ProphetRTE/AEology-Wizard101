...
from src import aeologygui
from src.aeologygui import GUIKeys
...
async def kill_tool(debug: bool):
	...
	gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.Close))
	await asyncio.sleep(0)
	await asyncio.sleep(0)
...
async def main():
	...
	async def toggle_speed_hotkey():
		...
		if not freecam_status:
			if speed_task is not None and not speed_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('SpeedhackStatus', 'Disabled')))
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('SpeedhackStatus', 'Enabled')))
	...
	async def toggle_combat_hotkey(debug: bool = True):
		...
		if not freecam_status:
			if combat_task is not None and not combat_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('CombatStatus', 'Disabled')))

			else:
				if debug:
					logger.debug(f'{toggle_auto_combat_key} key pressed, enabling auto combat.')
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('CombatStatus', 'Enabled')))
	...
	async def toggle_dialogue_hotkey(side_quests: bool = False):
		...
		if not freecam_status:
			if dialogue_task is not None and not dialogue_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('DialogueStatus', 'Disabled')))
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('DialogueStatus', 'Enabled')))
	...
	async def toggle_sigil_hotkey():
		...
		if not freecam_status:
			...
			if sigil_task is not None and not sigil_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('SigilStatus', 'Disabled')))
				...
				if questing_task is not None and not questing_task.cancelled():
					...
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('QuestingStatus', 'Disabled')))
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('SigilStatus', 'Enabled')))
	...
	async def toggle_questing_hotkey():
		...
		if not freecam_status:
			...
			if questing_task is not None and not questing_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('QuestingStatus', 'Enabled')))
	...
	async def toggle_auto_pet_hotkey():
		...
		if not freecam_status:
			...
			if auto_pet_task is not None and not auto_pet_task.cancelled():
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Auto PetStatus', 'Disabled')))
				auto_pet_task.cancel()
				auto_pet_task = None

			else:
				logger.debug(f'Enabling auto pet.')
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Auto PetStatus', 'Enabled')))
	...
	# 			gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Side QuestsStatus', 'Disabled')))		

	# 		else:
	# 			logger.debug('Enabling side quests.')
	# 			gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Side QuestsStatus', 'Enabled')))
		...
		if show_gui:
			...
			while True:
				if foreground_client:
					...
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Title', f'Client: {foreground_client.title}')))
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Zone', f'Zone: {current_zone}')))
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('x', f'X: {current_pos.x}')))
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('y', f'Y: {current_pos.y}')))
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('z', f'Z: {current_pos.z}')))
					gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Yaw', f'Yaw: {current_rotation.yaw}')))
				...
				# Eat as much as the queue gives us. We will be freed by exception
					while True:
						...
						match com.com_type:
							case aeologygui.GUICommandType.Close:
								await kill_tool_hotkey()

							case aeologygui.GUICommandType.ToggleOption:
								match com.data:
							...
							case aeologygui.GUICommandType.Copy:
								match com.data:
							...
							case aeologygui.GUICommandType.Teleport:
								match com.data:
							...
							case aeologygui.GUICommandType.CustomTeleport:
								if foreground_client:
							...
							case aeologygui.GUICommandType.EntityTeleport:
							...
							case aeologygui.GUICommandType.SelectEnemy:
								if foreground_client and await foreground_client.in_battle():
									...
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('stat_viewer', '\n'.join(enemy_stats))))
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindowValues, ('EnemyInput', names_list)))
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindowValues, ('AllyInput', names_list)))
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('EnemyInput', names_list[caster_i])))
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('AllyInput', names_list[target_i])))
									gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('SchoolInput', school_name)))

								else:
									logger.info('Last selected client is not currently in combat. You must be in combat to use the stat viewer.')

							case aeologygui.GUICommandType.XYZSync:
								await xyz_sync_hotkey()

							case aeologygui.GUICommandType.XPress:
								await x_press_hotkey()

							case aeologygui.GUICommandType.AnchorCam:
								if foreground_client:
									if freecam_status:
										await toggle_freecam_hotkey()
							...
							# case aeologygui.GUICommandType.SetPetWorld:
							...
							case aeologygui.GUICommandType.SetCamPosition:
								if foreground_client:
									if not freecam_status:
										await toggle_freecam_hotkey()
							...
							case aeologygui.GUICommandType.SetCamDistance:
								if foreground_client:
									camera = await foreground_client.game_client.elastic_camera_controller()
							...
							case aeologygui.GUICommandType.GoToZone:
								if foreground_client:
							...
							case aeologygui.GUICommandType.GoToWorld:
								if foreground_client:
									clients = [foreground_client]
									if com.data[0]:
										for c in background_clients:
							...
							case aeologygui.GUICommandType.GoToBazaar:
								if foreground_client:
									clients = [foreground_client]
									if com.data:
										for c in background_clients:
							...
							case aeologygui.GUICommandType.RefillPotions:
								if foreground_client:
									clients = [foreground_client]
									if com.data:
										for c in background_clients:
							...
							case aeologygui.GUICommandType.ExecuteFlythrough:
							...
							case aeologygui.GUICommandType.KillFlythrough:
							...
							case aeologygui.GUICommandType.ExecuteBot:
							...
							case aeologygui.GUICommandType.KillBot:
								if bot_task is not None and not bot_task.cancelled():
									bot_task.cancel()
									logger.debug('Bot Killed')
									bot_task = None

							case aeologygui.GUICommandType.SetPlaystyles:
							...
							case aeologygui.GUICommandType.SetScale:
				...
				gui_send_queue.put(aeologygui.GUICommand(aeologygui.GUICommandType.UpdateWindow, ('Auto PetStatus', bool_to_string(auto_pet_status))))
	...
	async def ban_watcher():
		known_ban = False
		try:
			rkey = winreg.OpenKeyEx(winreg.HKEY_CURRENT_USER, r"SOFTWARE\ProphetRTE\AEology", access=winreg.KEY_READ)
		...
		if not known_ban:
			...
			while ban_task.is_alive():
				await asyncio.sleep(1)
		try:
			rkey = winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, r"SOFTWARE\ProphetRTE\AEology", access=winreg.KEY_ALL_ACCESS)
			winreg.SetValueEx(rkey, "badboy", 0, winreg.REG_DWORD, 1)
		except:
			pass
		cMessageBox(None, "AEology has encountered a fatal error (Code 0C24). Please contact slackaduts on discord for more info.", "AEology error", 0x10 | 0x1000)
		quit(0)
